...
static rb_timer_item g_route_serv_timer;
...
{
	rbtimer_init(&g_route_serv_timer,route_serv_timer_callback,NULL,1000,0,0);
	netlib_register_timer(&g_route_serv_timer);
...
void CRouteConn::_HandleOnlineUserInfo(CImPdu* pPdu)
{
	...
	for (uint32_t i = 0; i < user_count; i++) {
		...
		_UpdateUserStatus(server_user_stat.user_id(), server_user_stat.status(), server_user_stat.app_id(), server_user_stat.domain_id());
	}
}
...
void CRouteConn::_HandleUserStatusUpdate(CImPdu* pPdu)
{
    ...
    bool pc_client;
    log("HandleUserStatusUpdate, status=%u, uid=%u, app_id=%u domain_id=%u", 
        user_status, user_id, app_id, domain_id);

    _UpdateUserStatus(user_id, user_status, app_id, domain_id);
    ...
    if (pUser)
    {
        ...
        if (user_status == USER_STATUS_OFFLINE)
        {
            ...
            if (pc_client && !pUser->IsPCClientLogin())
            {
                _BroadcastMsg(&pdu);
            }
        }
 ...
 */
void CRouteConn::_UpdateUserStatus(uint32_t user_id, uint32_t status, uint32_t app_id, uint32_t domain_id)
{
    CUserInfo* pUser = GetUserInfo(user_id);
    if (pUser) {
        if (pUser->FindRouteConn(this))
        {
            ...
            {
                pUser->RemoveClientType(app_id, domain_id);
            ...
            {
                pUser->AddClientType(app_id, domain_id);
            }
        }
        else
        {
    ...
    else
    {
        if (status != USER_STATUS_OFFLINE) {
            ...
            if (pUserInfo != NULL) {
                ...
                pUserInfo->AddClientType(app_id, domain_id);
