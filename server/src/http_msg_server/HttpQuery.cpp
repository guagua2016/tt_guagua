...
static rb_timer_item  g_http_query_timer;
...
CHttpQuery* CHttpQuery::GetInstance()
{
	if (!m_query_instance) {
		...
		rbtimer_init(&g_http_query_timer,http_query_timer_callback,NULL,1000,0,0);
		netlib_register_timer(&g_http_query_timer);
