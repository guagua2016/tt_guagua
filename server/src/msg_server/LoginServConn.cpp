...
static rb_timer_item g_login_server_conn_timer;
		...
		const char* msg_server_ip_addr2, uint16_t msg_server_port, uint32_t max_conn_cnt)
{
	...
	rbtimer_init(&g_login_server_conn_timer,login_server_conn_timer_callback,NULL,1000,0,0);
	netlib_register_timer(&g_login_server_conn_timer);
...
void CLoginServConn::Close()
{
	...
	if (m_handle != NETLIB_INVALID_HANDLE) {
		...
		log("Close connect to login_server");
...
void CLoginServConn::HandlePdu(CImPdu* pPdu)
{
	...
	case CID_LOGIN_REQ_LOGINOUT:
		...
		break;		
	default:
		loge("unknown cmd id=%d ", pPdu->GetCommandId());
		break;
	}
}

void CLoginServConn::_HandleKickUser(CImPdu* pPdu)
{
	...
	log("HandleKickUser, user_id=%u, app_id=%u, domain_id=%u, reason=%u. ", user_id, app_id, domain_id, reason);

	CImUser* pUser = CImUserManager::GetInstance()->GetImUserById(user_id);
	if (pUser) {
		pUser->KickOutSameClientType(app_id, domain_id, reason);
	}
}

void CLoginServConn::_HandleLoginOutRequest(CImPdu *pPdu)
{
	...
	log("HandleLoginOutRequest, user_id=%d, app_id=%d, domain_id=%d", user_id, app_id, domain_id);

    CImUser* pUser = CImUserManager::GetInstance()->GetImUserById(user_id);
    if (pUser) {
        CMsgConn* pMsgConn = pUser->GetMsgConnById(app_id, domain_id);
        if (pMsgConn && pMsgConn->isIOSClient()) {
            CDBServConn* pDBConn = get_db_serv_conn();
            if (pDBConn) {
                log("HandleLoginOutRequest, clear the device_token..");
        ...
        pUser->CloseMsgConn(app_id, domain_id);
    }
