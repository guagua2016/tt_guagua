...
static rb_timer_item g_push_server_conn_timer;
...
void init_push_serv_conn(serv_info_t* server_list, uint32_t server_count)
{
	...
	rbtimer_init(&g_push_server_conn_timer,push_server_conn_timer_callback,NULL,1000,0,0);
	netlib_register_timer(&g_push_server_conn_timer);
...
void build_ios_push_flash(string& flash, uint32_t msg_type, uint32_t from_id)
{
    ...
    else
    {
        ...
        {
            //FIXME
            //uint32_t user_id  = pImUser->GetUsrId();
            string msg_tmp;
            if (msg_type == IM::BaseDefine::MSG_TYPE_GROUP_AUDIO )
            {
                //msg_tmp.append(user_id);
