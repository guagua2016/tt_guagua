...
static rb_timer_item g_route_server_conn_timer;
...
void init_route_serv_conn(serv_info_t* server_list, uint32_t server_count)
{
	...
	rbtimer_init(&g_route_server_conn_timer,route_server_conn_timer_callback,NULL,1000,0,0);
	netlib_register_timer(&g_route_server_conn_timer);
...
void CRouteServConn::OnConfirm()
{
    ...
    for (list<user_stat_t>::iterator it = online_user_list.begin(); it != online_user_list.end(); it++) {
        ...
        server_user_stat->set_app_id(user_stat.app_id);
        server_user_stat->set_domain_id(user_stat.domain_id);
    }
    CImPdu pdu;
    pdu.SetPBMsg(&msg);
    pdu.SetServiceId(SID_OTHER);
