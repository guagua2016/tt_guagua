...
void CDBServConn::_HandleValidateResponse(CImPdu* pPdu)
{
    ...
    log("HandleValidateResp, user_id=%d, app_id:%d, domain_id:%d, result=%d", user_id, app_id, domain_id, result);
    
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserById(user_id);
    CMsgConn* pMsgConn = NULL;
    if (!pImUser) {
        loge("ImUser for user_id=%d not exist", user_id);
        return;
    } else {
        pMsgConn = pImUser->GetUnValidateMsgConn(attach_data.GetHandle());
        if (!pMsgConn || pMsgConn->IsOpen()) {
            loge("no such conn is validated, user_id=%s", user_id);
            return;
        }
    }
    ...
    if (result == 0) {
        ...
        pImUser->KickOutSameClientType(app_id, domain_id, IM::BaseDefine::KICK_REASON_DUPLICATE_USER, pMsgConn);
        
        CRouteServConn* pRouteConn = get_route_serv_conn();
        if (pRouteConn) {
            IM::Server::IMServerKickUser msg2;
            msg2.set_user_id(user_id);
            msg2.set_app_id(app_id);
            msg2.set_domain_id(domain_id);
            msg2.set_reason(1);
            CImPdu pdu;
            pdu.SetPBMsg(&msg2);
            pdu.SetServiceId(SID_OTHER);
        ...
        //log("uid: %d, app_id:%d, domain_id:%d", user_id, app_id, domain_id);
        pMsgConn->SetUserId(user_id, app_id, domain_id);
        pMsgConn->SetOpen();
        ...
        user_info_tmp->set_app_id(user_info.app_id());
        user_info_tmp->set_domain_id(user_info.domain_id());
...
#if 0 
    if (msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_TEXT || msg_type == IM::BaseDefine::MSG_TYPE_SINGLE_PRODUCT_CARD || msg_type == IM::BaseDefine::MSG_TYPE_GROUP_TEXT)
    {
        //msg_data =
        char* msg_out = NULL;
        uint32_t msg_out_len = 0;
		int32_t result = pAes->Decrypt(msg_data.c_str(), msg_data.length(), &msg_out, msg_out_len);
		if (result == 0)
        {
        ...
        {
            log("HandleGetDeviceTokenResponse, decrypt msg failed, result =%d ,from_id: %u, to_id: %u, msg_type: %u.", result, from_id, to_id, msg_type);
            return;
        }
        pAes->Free(msg_out);
    }
