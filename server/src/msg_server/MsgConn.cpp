...
#define LOG_MSG_STAT_INTERVAL			60000   //300000	// log message miss status in every 5 minutes;
...
static rb_timer_item  g_msg_conn_timer;
...
void init_msg_conn()
{
	...
	rbtimer_init(&g_msg_conn_timer,msg_conn_timer_callback,NULL,1000,0,0);
	netlib_register_timer(&g_msg_conn_timer);
...
void CMsgConn::SendUserStatusUpdate(uint32_t user_status)
{
    ...
    if (user_status == ::IM::BaseDefine::USER_STATUS_ONLINE) {
        ...
        msg2.set_app_id(m_app_id);
	msg2.set_domain_id(m_domain_id);
        CImPdu pdu2;
        pdu2.SetPBMsg(&msg2);
...
bool CMsgConn::isMobileClient() const
{
	switch (m_app_id) {
	case 1:  // B2C Android
	case 2:  // B2C iOS
	case 21: // JiuXiu Android
	case 22: // JiuXiu iOS
		return true;
	default:
		return false;
...
bool CMsgConn::isPCClient() const
{
	switch (m_app_id) {
	case 3: // B2C Web
	case 11: // GF Web
...
bool CMsgConn::isIOSClient() const
{
	switch (m_app_id) {
	case 2:  // B2C iOS
	case 22: // JiuXiu iOS
...
uint32_t CMsgConn::GetClientType() const
{
	switch (m_app_id) {
	case 1:
	case 21:
...
void CMsgConn::_HandleLoginRequest(CImPdu* pPdu)
{
	...
	result = IM::BaseDefine::REFUSE_REASON_INVALID_USER_ID;
	result_string = "Invalid user id";
    } else if (m_user_token.empty()) {
        result = IM::BaseDefine::REFUSE_REASON_INVALID_USER_TOKEN;
        result_string = "Invalid user token";
    }

    if (result) {
        log("HandleLoginReq, Error, result : %u  %s", result, result_string.c_str());
    ...
    log("HandleLoginReq, user_id=%d, app_id=%d, domain_id=%d status=%u, client=%s, ",
        m_user_id, m_app_id, m_domain_id, online_status, m_client_version.c_str());
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserById(m_user_id);
    if (!pImUser) {
        pImUser = new CImUser(m_user_id);
        CImUserManager::GetInstance()->AddImUserById(m_user_id, pImUser);
    }
    ...
    msg2.set_user_id(m_user_id);
    msg2.set_app_id(m_app_id);
    msg2.set_domain_id(m_domain_id);
    msg2.set_user_token(m_user_token);
...
{
	log("HandleLoginOutRequest, user_id=%d, app_id=%d, domain_id=%d", 
		m_user_id, m_app_id, m_domain_id);
...
void CMsgConn::_HandleKickPCClient(CImPdu *pPdu)
{
    ...
    if (!isMobileClient()) {
	log("HandleKickPCClient, user_id = %u, cmd must come from mobile client. ", m_user_id);
	return;
    }
    log("HandleKickPCClient, user_id = %u. ", m_user_id);
    
    CImUser* pImUser = CImUserManager::GetInstance()->GetImUserById(m_user_id);
    if (pImUser)
    {
        //TODO: FIXME
        pImUser->KickOutSameClientType(23, 1200, IM::BaseDefine::KICK_REASON_MOBILE_KICK, this);
    }
...
void CMsgConn::_HandleClientMsgData(CImPdu* pPdu)
{
	...
	if (g_log_msg_toggle) {
		log("HandleClientMsgData, %d->%d, msg_type=%u, msg_id=%u, add_id=%u, domain_id=%u", GetUserId(), to_session_id, msg_type, msg_id, m_app_id, m_domain_id);
	}
...
void CMsgConn::_HandleClientMsgDataAck(CImPdu* pPdu)
{
    ...
    if (session_type == IM::BaseDefine::SESSION_TYPE_SINGLE)
    {
        ...
        log("###Start DelFromSendList msg_id=%d, session_id=%d,handle=%d",msg_id, session_id,this->m_handle);
...
#else
    loge("IMChangeAvatarReq is deprecated");
#endif
}
...
uint32_t CMsgConn::GetClientTypeFlag() const
{
