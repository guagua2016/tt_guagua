...
public:
    ...
    uint32_t getAppId() const { return m_app_id; }
    uint32_t getDomainId() const { return m_domain_id; }
    void SetUserId(uint32_t user_id, uint32_t app_id, uint32_t domain_id) { 
        m_user_id = user_id;
        m_app_id = app_id;
    ...
    bool IsKickOff() const { return m_bKickOff; }
    void SetOnlineStatus(uint32_t status) { m_online_status = status; }
    uint32_t GetOnlineStatus() const { return m_online_status; }
    bool isMobileClient() const;
    bool isPCClient() const;
    bool isIOSClient() const;
    uint32_t GetClientType() const;
    uint32_t GetClientTypeFlag() const;
