...
//		printf("Server Version: MsgServer/%s\n", VERSION);
	...
	char* cert_file = config_file.GetConfigName("CertFile");
	char* key_file = config_file.GetConfigName("PrivateKeyFile");
	...
	if (cert_file == NULL || key_file == NULL) {
		loge("config file miss certificate/key files");
		return -1;
	}
	...
	for (uint32_t i = 0; i < listen_ip_list.GetItemCnt(); i++) {
		ret = netlib_ssl_listen(listen_ip_list.GetItem(i),
			listen_port,
			cert_file,
			key_file,
			NULL,
