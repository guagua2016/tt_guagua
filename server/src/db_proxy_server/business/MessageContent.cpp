...
//                    log("userId=%u, peerId=%u, msgId=%u", nUserId, nPeerId, it->msg_id());
    ...
    void sendMessage(CImPdu* pPdu, uint32_t conn_uuid)
    {
        ...
        if(msg.ParseFromArray(pPdu->GetBodyData(), pPdu->GetBodyLength()))
        {
			...
			uint32_t nDomainId = msg.domain_id();
            ...
            if (IM::BaseDefine::MsgType_IsValid(nMsgType))
            {
                if(nMsgLen != 0)
                {
                    ...
                    } else if(nMsgType== IM::BaseDefine::MSG_TYPE_SINGLE_TEXT || nMsgType == IM::BaseDefine::MSG_TYPE_SINGLE_PRODUCT_CARD) {
                        if (nFromId != nToId) {
                            ...
                            {
                                //nSessionId = CSessionModel::getInstance()->addSession(nToId, nAppId, nDomainId, nFromId, IM::BaseDefine::SESSION_TYPE_SINGLE);
                            ...
                            if(nSessionId != INVALID_VALUE && nRelateId != INVALID_VALUE)
                            {
                                ...
                                {
                                    log("msgId is invalid. fromId=%u, appId=%u, domainId=%u, toId=%u, nRelateId=%u, nSessionId=%u, nMsgType=%u", nFromId, nAppId, nDomainId, nToId, nRelateId, nSessionId, nMsgType);
                                }
                            }
                            else{
                                log("sessionId or relateId is invalid. fromId=%u, appId=%u, domainId=%u, toId=%u, nRelateId=%u, nSessionId=%u, nMsgType=%u", nFromId, nAppId, nDomainId, nToId, nRelateId, nSessionId, nMsgType);
                            }
                        }
                        else
                        {
                            log("send msg to self. fromId=%u, appId=%u, domainId=%u, toId=%u, msgType=%u", nFromId, nAppId, nDomainId, nToId, nMsgType);
                        }
                    ...
                    log("fromId=%u, ,appId=%u, domainId=%u, toId=%u, type=%u, msgId=%u, sessionId=%u", nFromId, nAppId, nDomainId, nToId, nMsgType, nMsgId, nSessionId);
