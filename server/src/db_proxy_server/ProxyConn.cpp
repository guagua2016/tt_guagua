...
static rb_timer_item  g_exit_callback_timer;
static rb_timer_item  g_callback_timer;
...
static void sig_handler(int sig_no)
{
	if (sig_no == SIGTERM) {
		...
		rbtimer_init(&g_exit_callback_timer,exit_callback,NULL,4000,0,1);
		netlib_register_timer(&g_exit_callback_timer);
	}
}
...
int init_proxy_conn(uint32_t thread_num)
{
	...
	rbtimer_init(&g_callback_timer,proxy_timer_callback,NULL,1000,0,0);
	return netlib_register_timer(&g_callback_timer);
