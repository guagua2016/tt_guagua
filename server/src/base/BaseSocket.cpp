...
int CBaseSocket::Listen(const char* server_ip, uint16_t port, callback_t callback, void* callback_data)
{
	...
	_setSoLinger(false, 0);
	_setReuseAddr(true);
	setNonblock(true);
	_setIntOption(SO_KEEPALIVE, 1);
	_setIntOption(SO_SNDBUF, 640000);
	_setIntOption(SO_RCVBUF, 640000);
	setTcpNoDelay(true);
	...
	if (ret == SOCKET_ERROR)
	{
		log("bind failed, err_code=%d ,  err=%s", _GetErrorCode(),strerror(errno));
...
void CBaseSocket::OnRead()
{
	...
	else
	{
		...
		{
            log("base socket ioctlsocket failed, err_code=%d, %s", _GetErrorCode(), strerror(errno));
...
#endif
		if (error) {
			log("getsockopt failed, err_code=%d, %s", _GetErrorCode(), strerror(errno));
			m_callback(m_callback_data, NETLIB_MSG_CLOSE, (net_handle_t)m_socket, NULL);
		} else {
...
{
	bool rc = _setIntOption(SO_SNDBUF, send_size);
	if (rc == false) {
...
{
	bool rc = _setIntOption(SO_RCVBUF, recv_size);
	if (rc == false) {
		log("set SO_RCVBUF failed for fd=%d", m_socket);
	}
}

bool CBaseSocket::_setIntOption(int option, int value)
{
	bool rc = false;
...
bool CBaseSocket::_setTimeOption(int option, int sec, int usec)
{
	...
	if (_checkSocketHandle()) {
		rc = (setsockopt(m_socket, SOL_SOCKET, option, (const void*)&tv, sizeof(tv)) == 0);
	}
	return rc;
}
...
bool CBaseSocket::_setSoLinger(bool doLinger, int seconds)
{
	bool rc = false;
...
void CBaseSocket::_setReuseAddr(bool on)
{
	int reuse = on ? 1 : 0;
	
	if (false == _setIntOption(SO_REUSEADDR, reuse)) {
...
void CBaseSocket::setTcpNoDelay(bool noDelay)
{
	...
	if (ret == SOCKET_ERROR)
	{
		log("_setTcpNoDelay failed, err_code=%d", _GetErrorCode());
	}
}
