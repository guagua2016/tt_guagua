...
namespace tt {
  namespace common {
    ...
    int EasyHelper::on_connect_cb(easy_connection_t *c)
    {
      ...
      c->life_idle = 1;
      DEBUG("connection established: %s", easy_connection_to_str(c).c_str());
      return EASY_OK;
    }

    int EasyHelper::on_disconnect_cb(easy_connection_t *c)
    {
    	DEBUG("disconnect: %s, c->pool->ref: %ld, c->doing_request_count: %d",
    ...
    {
    	DEBUG("disconnect for %d seconds idleness: %s, c->last_time: %lf",
    ...
    int EasyHelper::cleanup_cb(easy_request_t *r, void *packet)
    {
      ...
      if (r != NULL)
      {
        if (r->ipacket != NULL)
        {
          delete (void*)r->ipacket;
          r->ipacket = NULL;
        }
        if (r->opacket != NULL)
        {
          delete (void*)r->opacket;
          r->opacket = NULL;
        }
      }
      return EASY_OK;
    ...
    bool EasyHelper::is_alive(easy_addr_t &addr, easy_io_t *eio)
    {
      ...
      {
        ERROR("establish socket failed: %s", strerror(errno));
        return false;
      }
    ...
    bool EasyHelper::wait_connected(int fd, int timeout/*ms*/)
    {
      ...
      {
    	  ERROR("epoll_ctl::EPOLL_CTL_ADD failed: %m");
        close(ep);
        return false;
      }
      do
      {
        ...
        if (n == -1)
        {
        	ERROR("epoll_wait failed: %d, %m", fd);
          break;
        }
        if (n == 0)
        {
        ...
        int retry)
    {
      if (eio == NULL)
      {
        eio = tt::common::EasyHelper::eio;
      }
      if (handler == NULL)
      {
        handler = tt::common::EasyHelper::handler;
      }
      if (eio == NULL || handler == NULL || packet == NULL)
      {
    	  ERROR("eio, handler, or packet is null");
        return EASY_ERROR;
      }
      ...
      if (s == NULL)
      {
    	  ERROR("create easy session failed");
        return EASY_ERROR;
      }
