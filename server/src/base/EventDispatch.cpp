...
#else
	m_epfd = epoll_create(MAX_EVENTS_FD);
	if (m_epfd == -1)
	{
		log("epoll_create failed");
	}

	m_pEvents = (struct epoll_event*)malloc(MAX_EVENTS_FD * sizeof(struct epoll_event));
#endif
}
...
void CEventDispatch::TTAddEvent(SOCKET fd, uint8_t socket_event)
{
	struct epoll_event ev;
	ev.events = EPOLLIN | EPOLLOUT | EPOLLET | EPOLLPRI | EPOLLERR | EPOLLHUP;
	ev.data.fd = fd;
...
void CEventDispatch::TTRemoveEvent(SOCKET fd, uint8_t socket_event)
{
...
void CEventDispatch::ProcessEventsAndTimers(uint32_t wait_timeout)
{
	int nfds = epoll_wait(m_epfd,m_pEvents,MAX_EVENTS_FD,wait_timeout);
	for (int i = 0; i < nfds; i++)
	{
		int ev_fd = m_pEvents[i].data.fd;
		CBaseSocket* pSocket = FindBaseSocket(ev_fd);
		if (!pSocket)
			continue;

      #ifdef EPOLLRDHUP
      if (m_pEvents[i].events & EPOLLRDHUP)
        {
		...
		if (m_pEvents[i].events & (EPOLLPRI | EPOLLERR | EPOLLHUP))
		{
...
void CEventDispatch::StartDispatch(uint32_t wait_timeout)
{
	...
	while (running)
	{
		...
		nfds = epoll_wait(m_epfd,m_pEvents,MAX_EVENTS_FD,wait_timeout);
		for (int i = 0; i < nfds; i++)
		{
			int ev_fd = m_pEvents[i].data.fd;
			CBaseSocket* pSocket = FindBaseSocket(ev_fd);
...
uintptr_t CEventDispatch::EventFindTimer(void)
{
	 uintptr_t      timer;
    rbtree_node_t  *node, *root, *sentinel;

    if (m_event_timer_rbtree.root == &m_event_timer_sentinel) {
        return -1;
     }
    ...
    node = rbtree_min(root, sentinel);
    timer = (uintptr_t)(node->key - current_msec);
