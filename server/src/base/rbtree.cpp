...
void inorder_walkthrough(rbtree_node_t *root)
{
	if (root)
	{
		printf("%d	", root->key);
		inorder_walkthrough(root->left);
		inorder_walkthrough(root->right);
	}
}
...
int count_tree_node(rbtree_node_t *root,rbtree_node_t *sentinel)
{
	if (root != sentinel)
	{
		return (count_tree_node(root->left,sentinel)+count_tree_node(root->right,sentinel)+1);
	}
...
int hight_tree(rbtree_node_t *root,rbtree_node_t *sentinel)
{
	...
	left = hight_tree(root->left,sentinel);
	right = hight_tree(root->right,sentinel);
